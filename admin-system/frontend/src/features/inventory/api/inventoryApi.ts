 ...
 */
export async function cancelInbound(payload: CancelInboundPayload): Promise<StockInboundRecord> {
  const res = await fetch(`${API_BASE}/api/v1/inventory/inbound/cancel`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  });
