...
func (s *InventoryService) ProcessStockInbound(ctx context.Context, req domain.ProcessInboundRequest) (*domain.StockInboundRecord, error) {
	if s.db == nil {
	...
	if totalPrice.IsZero() || totalPrice.LessThanOrEqual(decimal.Zero) {
	...
	incomingConsumptionUnitCost := totalPrice.Div(incomingConsumptionQty)
	costPerPurchaseUnit := totalPrice.Div(req.QuantityReceived)
...
func (s *InventoryService) CancelStockInbound(ctx context.Context, req domain.CancelInboundRequest) (*domain.StockInboundRecord, error) {
	if s.db == nil {
		return nil, fmt.Errorf("database not initialized")
	}
...
func (s *InventoryService) UpdateMaterialDirect(ctx context.Context, id string, req domain.UpdateMaterialRequest) (*domain.Material, error) {
...
func (s *InventoryService) IntakeInkBottle(ctx context.Context, req domain.IntakeInkBottleRequest) (*domain.InkBottleInventory, error) {
	...
	if err := s.inboundRepo.CreateInkBottle(ctx, item); err != nil {
		return nil, err
	}
	return item, nil
}
