...
import { Package, Droplets, Tag, DollarSign, Boxes, Truck, PhoneCall } from 'lucide-react';
...
export default function InkDetailsView({ item }) {
  const formatLAK = (num) => new Intl.NumberFormat('lo-LA', { style: 'currency', currency: 'LAK' }).format(num || 0).replace('LAK', '₭');

  const itemName = item?.itemName || item?.name || 'Unassigned Ink Item';
  const inkType = item?.inkType || item?.type || 'Ink / Toner';
  const unitPrice = item?.unitPrice || item?.costPerUnit || item?.purchasePrice;
  const qty = item?.qty || item?.quantity || item?.initialQty;
  const unitName = item?.unitName || item?.purchaseUnit || 'Bottle';
  const supplierName = item?.supplierName || '-';
  const supplierContact = item?.supplierContact || item?.note || '-';
  const bottleVolume = item?.bottleVolumeMl || item?.capacityMl;

  return (
    <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
      <div className="border-b border-slate-100 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <InboundStatusBadge categoryType="Materials" materialType="Ink / Toner" labelOverride="ໝວດ A: ໝຶກພິມ (Ink / Toner)" />
      ...
      <div className="space-y-6">
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
          ...
          <span>ລາຍລະອຽດໝຶກພິມ (Ink & Consumable Specifications)</span>
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-1 shadow-sm">
            ...
            <p className="text-sm font-black text-slate-900">ໝຶກພິມ (Ink / Toner)</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-1 shadow-sm">
            <div className="flex items-center gap-2 text-slate-500"><Droplets className="w-4 h-4 text-sky-600" /><span className="text-[10px] font-black uppercase">ປະເພດ/ຄວາມຈຸ (Ink Type/Vol)</span></div>
            <p className="text-sm font-black text-slate-900">{inkType} {bottleVolume ? `(${bottleVolume} ml)` : ''}</p>
          </div>
