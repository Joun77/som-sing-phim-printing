...
import { Package, Tag, DollarSign, Boxes, Truck, PhoneCall } from 'lucide-react';
...
export default function DynamicMaterialDetailsView({ item }) {
  const formatLAK = (num) => new Intl.NumberFormat('lo-LA', { style: 'currency', currency: 'LAK' }).format(num || 0).replace('LAK', '₭');

  const categoryName = item?.materialType || item?.category || item?.subCategory || item?.inboundType || 'Materials & Supplies';
  const itemName = item?.itemName || item?.name || 'Unassigned Material Item';
  const unitPrice = item?.unitPrice || item?.costPerUnit || item?.purchasePrice;
  const qty = item?.qty || item?.quantity || item?.initialQty;
  const unitName = item?.unitName || item?.purchaseUnit || 'Units';
  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
      <div className="border-b border-slate-100 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <InboundStatusBadge categoryType="Materials" materialType={categoryName} labelOverride={`ໝວດ A: ວັດສະດຸ [${categoryName}]`} />
      ...
      <div className="space-y-6">
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
          ...
          <span>ລາຍລະອຽດຂໍ້ມູນວັດສະດຸ (Material Item Specifications)</span>
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-1 shadow-sm">
            ...
            <p className="text-sm font-black text-slate-900">{categoryName}</p>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-1 shadow-sm sm:col-span-2 lg:col-span-4">
