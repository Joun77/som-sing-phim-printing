...
// UpdatePricingTemplate updates an existing template record
func (s *PricingService) UpdatePricingTemplate(ctx context.Context, id string, payload domain.UpdatePricingTemplatePayload) (*domain.ProductPricingTemplate, error) {
	existing, err := s.GetPricingTemplateByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("pricing template %s not found: %w", id, err)
	}
	...
	baseline := existing.BaselineCoveragePercent
	if payload.BaselineCoveragePercent != nil {
		baseline = *payload.BaselineCoveragePercent
	}

	multiplier := existing.CoverageSurchargeMultiplier
	if payload.CoverageSurchargeMultiplier != nil {
		multiplier = *payload.CoverageSurchargeMultiplier
	}

	minQty := existing.MinOrderQuantity
	if payload.MinOrderQuantity != nil {
		minQty = *payload.MinOrderQuantity
	}

	minPrice := existing.MinTotalPrice
	if payload.MinTotalPrice != nil {
		minPrice = *payload.MinTotalPrice
	}
	...
	addonRates := existing.AddonRates
	if payload.AddonRates != nil {
		addonRates = *payload.AddonRates
	}
	addonJSON, err := json.Marshal(addonRates)
	if err != nil {
		addonJSON = []byte("{}")
	}

	query := `
		UPDATE product_pricing_templates SET
			name = $1,
			material_id = $2,
			baseline_coverage_percent = $3,
			coverage_surcharge_multiplier = $4,
	...
	err = s.db.QueryRowContext(ctx, query,
		name, materialID, baseline, multiplier,
		minQty, minPrice, addonJSON, isActive, id,
	).Scan(
		&t.ID, &t.Name, &t.MaterialID, &t.BaselineCoveragePercent, &t.CoverageSurchargeMultiplier,
		&t.MinOrderQuantity, &t.MinTotalPrice, &returnedAddonJSON, &t.IsActive,
		&t.CreatedAt, &t.UpdatedAt,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to update pricing template: %w", err)
	}
